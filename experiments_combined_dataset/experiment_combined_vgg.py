...
from helpers import (
    ...
    run_model, generate_eigencam_overlays, infer_flatten_size
)
...
TRAIN_FRAC      = 0.80     # of remaining 90%, train = 80% (val = 20%)
STAGE1_EPOCHS   = 10
STAGE2_EPOCHS   = 10
STAGE3_EPOCHS   = 10
OUTPUT_DIR      = "./outputs_experiment_combined_vgg16_coarse"
SEED            = 42
...
def make_splits(root, tf_train, tf_eval, stratify_subfolder=True):
    ...
    if stratify_subfolder:
        # stratify by the macro-class folder name
...
# ─── VGG wrapper to expose features + head ────────────────────────────────────
...
def make_vgg(num_classes):
    feats = models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1).features
    # freeze features initially
    for p in feats.parameters(): p.requires_grad = False
    # infer flatten size at runtime
    flat_dim = infer_flatten_size(feats.to(device), input_shape=(1,3,IMG_SIZE,IMG_SIZE), device=device)
    head = nn.Sequential(
        nn.Linear(flat_dim, 256), nn.ReLU(), nn.Dropout(0.5),
        nn.Linear(256, num_classes)
    )
    model = VGGModel(feats, head).to(device)
    # CAM hook (Eigen-CAM expects model.features to be the conv stack)
    object.__setattr__(model, 'features', model.features)
    return model
...
model = make_vgg(len(coarse_classes))
crit  = nn.CrossEntropyLoss()

# Stage 1: head-only (classifier)
...
# Stage 2: unfreeze last conv block (conv5_x ≈ features[24:])
...
print("✅ Coarse-only VGG16 experiment complete. Outputs in", OUTPUT_DIR)
