...
from helpers import (
    set_seed, make_dataloader, train_validate, save_curve, run_model
)
...
STAGE2_EPOCHS   = 10
STAGE3_EPOCHS   = 10
OUTPUT_DIR      = "./outputs_experiment_combined_dinovit_coarse"
SEED            = 42
...
def make_splits(root, tf_train, tf_eval, stratify_subfolder=True):
    ...
    idxs = list(range(len(raw.samples)))
    if stratify_subfolder:
        stratify_labels = [os.path.normpath(p).split(os.sep)[-2] for p,_ in raw.samples]
    else:
        stratify_labels = [raw.targets[i] for i in idxs]

    tv, test = train_test_split(idxs, test_size=TEST_FRAC,
                                stratify=stratify_labels, random_state=SEED)
    tv_labels = [stratify_labels[i] for i in tv]
    train, val = train_test_split(tv, test_size=(1-TRAIN_FRAC),
...
# ─── MODEL FACTORY (DINOv2 ViT-S/14) ──────────────────────────────────────────
def make_dinovit(num_classes):
    ...
    # expose a conv-like feature handle for Eigen-CAM helper
    object.__setattr__(model, 'features', nn.ModuleList([backbone.patch_embed.proj]))
    return model
...
model = make_dinovit(len(coarse_classes))
crit  = nn.CrossEntropyLoss()

# Stage 1: head-only (freeze backbone, train head)
...
for ep in range(1, STAGE2_EPOCHS+1):
    hh = train_validate(model, opt2, crit, 1, f"Coarse-Stage2-Ep{ep}",
                        train_coarse, val_coarse, device, coarse_classes)
    for k in h2: h2[k].append(hh[k][0]); sched2.step()

# Stage 3: full fine-tune
for p in model[0].parameters(): p.requires_grad = True
...
for ep in range(1, STAGE3_EPOCHS+1):
    hh = train_validate(model, opt3, crit, 1, f"Coarse-Stage3-Ep{ep}",
                        train_coarse, val_coarse, device, coarse_classes)
    for k in h3: h3[k].append(hh[k][0]); sched3.step()
...
generate_eigencam_overlays(model, test_coarse.dataset, coarse_classes,
                           "Coarse", device, coarse_dir)

print("✅ Coarse-only DINOv2 ViT-S/14 experiment complete. Outputs in", OUTPUT_DIR)
