...
STAGE2_EPOCHS   = 10
STAGE3_EPOCHS   = 10
OUTPUT_DIR      = "./outputs_experiment_combined_convnext_coarse"
SEED            = 42
...
# ─── MODEL FACTORY (ConvNeXt-Base) ─────────────────────────────────────────────
def make_convnext_base(num_classes):
    model = timm.create_model('convnext_base', pretrained=True, num_classes=num_classes)
    # Expose stages for Eigen-CAM
    object.__setattr__(model, 'features', model.stages)
...
model = make_convnext_base(len(coarse_classes))
...
# Stage 2: unfreeze last 3 ConvNeXt stages
for stage in model.stages[-3:]:
...
print("✅ Coarse-only ConvNeXt-Base experiment complete. Outputs in", OUTPUT_DIR)
